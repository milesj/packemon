...
- Each package must contain a `package.json` (duh).
...
Both the `outDir` and `declarationDir` settings should be updated to `dts`, and should not be set to
`lib`, `build`, or some other variant. This is especially true if using project references.
